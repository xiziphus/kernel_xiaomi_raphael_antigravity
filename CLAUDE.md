...
- **`reboot,bpfloader-failed` on KameOS is a VERSION GATE, not a missing BPF feature.** netbpfload refuses to run below a kernel version tied to the ROM's release: `Android U requires kernel 4.14 / V requires 4.19 / 25Q2 requires 5.4`, and this ROM is 25q2. It checks `uname()`'s release *before* looking at any BPF feature, so map types, helpers, attach types, BTF and the `bpf_attr` layout are never reached. KameOS's own kernel passes by lying: disassembling the shipped Image shows `strncmp(current->comm, ...)` against `bpfloader`/`netbpfload`/`netd` and then `strcpy(tmp.release, "5.4.186")`. `scripts/fake_uname_bpfloader.py` reproduces it. **The claim is a promise** -- claiming 5.4 makes the loader then demand `CGROUP_SOCK_ADDR` (4.19 progs) and `CGROUP_SOCKOPT` (5.4 progs), so the tree needs those hooks too. Full detail in [docs/KAMEOS_DOCKER_JOURNEY.md](docs/KAMEOS_DOCKER_JOURNEY.md) §15.
- **A clean reboot WIPES ramoops on this device; only a panic preserves it.** Matching the ROM's pstore geometry is necessary but useless on its own -- verified by writing a marker to `/dev/kmsg` on KameOS's own kernel and rebooting. Add `qcom,force-warm-reboot;` to the `qcom,pshold` node (`msm-poweroff.c` already reads it and picks `PON_POWER_OFF_WARM_RESET`). This is why every failed boot before 2026-08-18 was silent.
- **Zundamon's own source is public at `HeliumStudio-Dev/kernel_xiaomi_raphael@oss-base` — it already contains the uname spoof.** Its `kernel/sys.c` carries the exact `strncmp(current->comm, "bpfloader"/"netbpfload"/"netd")` + `strcpy(tmp.release, "5.4.186")` hunk that was reverse-engineered out of the ROM's binary, plus a `current_uid().val == 0` guard the disassembly did not reveal. It is 4.14.356 (KameOS's sublevel), 8/8 cgroup hooks, real DEVMAP_HASH, `btf.c`, modern `bpf_attr`. **But it is the OSS/lineage variant, not the ROM's build**: its `raphael_defconfig` is `-perf`, it lacks `CONFIG_MACH_XIAOMI`, `MACH_XIAOMI_SM8150`, `MIHW`, `MILLET`, `SDCARD_FS`, `TOUCHSCREEN_XIAOMI_TOUCHFEATURE`, and `drivers/input/touchscreen/xiaomi/` does not exist in the tree at all. The ROM kernel has `xiaomi_touch` symbols and the ROM mounts `sdcardfs`, so KameOS's kernel is built from a MIUI sibling that is *not* published. Use oss-base as the **BPF/uname donor**, and a MIUI-lineage tree (Rikka, bool-x) as the base. **Tested 2026-08-19: oss-base powers the device off before init on KameOS** — the openela/AOSP-lineage rule holds for Helium's OSS tree too, so it is a donor and never a base.
- **A tree can be tested with no log channel at all: check `CONFIG_PSTORE_RAM` before blaming ramoops.** `tingyuwuxin/kernel_xiaomi_raphael@rikka-v5`'s `raphael_defconfig` contains **no PSTORE symbol whatsoever**, so two Rikka test boots reached netbpfload, failed, and left `/sys/fs/pstore` empty — which read as a fourth bug in `scripts/xiaomi_ramoops.py`'s DT node. The DT node was correct. bool-x and oss-base both enable PSTORE in-tree, which is why only Rikka went dark. The CI fragment now forces `PSTORE/PSTORE_RAM/PSTORE_CONSOLE/PSTORE_PMSG` and the pre-build check gates on them.
- **Rikka is 3/8 on cgroup attach types, and its enum names the gap exactly.** `rikka-v5`'s `enum bpf_attach_type` stops at `BPF_CGROUP_INET6_POST_BIND` (13): it has bind4/6 and connect4/6 but **not** `UDP4/6_SENDMSG`, `UDP4/6_RECVMSG`, `CGROUP_GETSOCKOPT`, `CGROUP_SETSOCKOPT`. Those are precisely the `netd.o` programs that a claimed 5.4 makes non-skippable, so Rikka cannot pass netbpfload unmodified. Claiming exactly **5.4.186** (not higher) is also what keeps the 5.9-gated `cgroupsockrelease` program skipped.
- **raphael boots system-as-root, so a custom kernel comes up with NO ROOT and says nothing about it.** The bootloader passes `skip_initramfs`; the kernel mounts `/system` as root and ignores the initramfs entirely, so `magiskinit` -- which lives only in the ramdisk -- never runs. The device boots perfectly, `su` is simply absent, and no log anywhere mentions it. Magisk's historical fix is a hexpatch of that one string to `want_initramfs` (same length, in place); KameOS's own kernel already carries it (`skip_initramfs=0 want_initramfs=2` vs our `1/0`). **Magisk's current `boot_patch.sh` no longer applies it** -- running the device's own copy over our image left the string untouched and booted equally rootless. `scripts/want_initramfs.py` does it, and `repack_kameos.sh` calls it on every image. Diagnosed by control: repacking KameOS's OWN kernel through the same pipeline (same ramdisk, cmdline, header) produced working root, leaving the kernel binary as the only variable.
- **Making a BPF program LOAD is half the job; netd then ATTACHES it, and that is a different switch.** `bpf_prog_attach()` and `bpf_prog_detach()` each carry their own `attach_type -> prog_type` mapping that defaults to `-EINVAL`, separate from `bpf_prog_load_check_attach_type()`. netd treats an attach failure as fatal (`libnetd_updatable_init: Failed: (22) ... attach failed` -> `SIGABRT`), and a dead netd is a zygote crash-loop: the device reaches adbd and even runs dockerd, then hangs at the boot animation with no `wlan0`. Any new attach type must appear in all three places.
- **Check `/proc/version` before trusting any on-device result.** A failed `fastboot boot` and an init-triggered reboot both land back on the ROM's own kernel, where `ro.boot.ktest` is empty. An `addrtype`-missing iptables failure was chased for some time before `uname -v` showed the ROM's build stamp -- the test kernel had been gone for several minutes.
- **KameOS = Zundamon = the Rikka lineage.** The Helium changelog says *"Renamed kernel name from Rikka to Zundamon"*, and the ROM kernel's kallsyms carry `cass_` and EEVDF symbols from that same changelog. `tingyuwuxin/kernel_xiaomi_raphael@rikka-v5` boots KameOS (reaches netbpfload) and already has `btf.c`, modern `bpf_attr` and 4/8 cgroup hooks -- a better base than bool-x (0/8).
- **[SUPERSEDED, kept for the reasoning] A MIUI-lineage kernel boots KameOS but dies at `netbpfload`, because stock 4.14 BPF is missing the cgroup hooks `netd.o` needs.** `onettboots/bool-x_xiaomi_raphael@16-HyperMiui` is the first custom kernel to reach init on this ROM; it then reboots with `reboot,bpfloader-failed`. The real loader is `/apex/com.android.tethering/bin/netbpfload`, and `netd.o` is marked **`critical`** while requiring `bind4/6`, `connect4/6`, `sendmsg4/6`, `recvmsg4/6`, `getsockopt`, `setsockopt`, `cgroupsockrelease` — hooks from kernels **4.17–5.10**. bool-x's `include/uapi/linux/bpf.h` defines only `BPF_CGROUP_INET_INGRESS/EGRESS/INET_SOCK_CREATE/SOCK_OPS`. **This is not caused by our Docker options** — a `stock_only=true` control build (no `docker.config`, no tracing) fails identically — and none of bool-x's 70 branches carries modern BPF. The openela lineage (Helium `oss-base`, SOVIET, InfinityX) *does* have the full backport but **powers the device off before init** on KameOS. The two halves are in different trees; the fix is to transplant openela's BPF into bool-x (both are 4.14.356 — see the memory note for the exact file list), or a minimal ~6-commit port of just the sock_addr/sockopt hooks.
