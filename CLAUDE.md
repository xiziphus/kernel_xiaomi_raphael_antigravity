...
- **Kernels do not transplant across ROM tree lineages, even at the same version.** On KameOS v3.303 (HyperOS 3, `4.14.356-Zundamon-v3.0`) five Docker-capable kernels from three trees all failed to boot, while a control — KameOS's own kernel repacked by the same script — booted. KameOS is MIUI/CAF lineage (`BUILD_ARM64_APPENDED_DTB_IMAGE`, `BOOT_INFO`, `ANDROID_LOW_MEMORY_KILLER`, no `*_CPU_MASK`); the openela/SOVIET trees are `BUILD_ARM64_DT_OVERLAY` with `*_CPU_MASK`. Grafting the ROM's own DTB onto a foreign kernel does not rescue it. Match the tree lineage, not just `uname -r`.
